...
if( len(sys.argv) != 3 ):
        sys.exit("Error: Arguments not specified.\nTry again with <Pixel Brightness threshold> <Points distance>\neg. python ibvs.py 100 50")
...
for frame in camera.capture_continuous(rawCapture, format="bgr", use_video_port=True):
	...
	c,r,w,h = 1200,450,350,350
	track_window = (c,r,w,h)
	
	# Create mask and normalized histogram
	roi = cframe_rgb[r:r+h, c:c+w]
...
for frame in camera.capture_continuous(rawCapture, format="bgr", use_video_port=True):
	...
	hsv = cv2.cvtColor(cframe_rgb, cv2.COLOR_BGR2HSV)
	dst = cv2.calcBackProject([hsv], [0], roi_hist, [0,180], 1)
	ret, track_window = cv2.meanShift(dst, track_window, term_crit)
	x,y,w,h = track_window
	cv2.rectangle(cframe_rgb, (x,y), (x+w,y+h), 255, 2)
...
#print "Desired cam velocity :: X:", int(v_cam[0]), "Y: ", int(v_cam[0][1]), "Z: ", int(v_cam[0][2])
		...
		print "Des Cam Vel in the Bodyframe: ", v_c_des_B
		...
		print "Here i am rock you like a huricane!!! <3 "
