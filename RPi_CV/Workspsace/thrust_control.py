...
for frame in camera.capture_continuous(rawCapture, format="bgr", use_video_port=True):
	...
	cframe_gray = cframe_gray[0:480, 0:639]
	...
	s_prev = s
		...
		p1_value = cframe_gray[s[0][0]+10][s[0][1]]
		p2_value = cframe_gray[s[1][0]+10][s[1][1]]
		p3_value = cframe_gray[s[2][0]+10][s[2][1]]
		p4_value = cframe_gray[s[3][0]+10][s[3][1]]
...
#print "Desired cam velocity :: X:", int(v_cam[0]), "Y: ", int(v_cam[0][1]), "Z: ", int(v_cam[0][2])
		...
		for p in range(0, 7, 2):
			cv2.circle(cframe_rgb, (s_raw[p], s_raw[p+1]), 8, (0, 255, 0), -1)
			#print s[p][1]
	
		# Display with red circles the desired points
		for l in range(0, 7, 2):
			cv2.circle(cframe_rgb, (s_des[l], s_des[l+1]), 5, (0, 0, 255), -1)
			#print s_des[l+1]
	
		# 20 for X and 80 for Y
		cv2.circle(cframe_rgb, (20, 80), 15, (150, 77, 10), -1)
		#cv2.imwrite('output.jpg', cframe_rgb)
		cv2.imshow('Image Frame', cframe_rgb)
	
	else:
		print "[WARNING] More or less than 4 points found. Total number: ", len(s)
		print s

		#cv2.imwrite('logframe-input.jpg', cframe_gray)
		
		# Display the current and desired points in a window
		# Paint red and green ricles for the current and desired points 
		for p in range(0, len(s)):
			cv2.circle(cframe_rgb, (s[p][1], s[p][0]), 5, (0, 255, 0), -1)
		cv2.imshow('Error Frame', cframe_rgb)
		#cv2.imwrite('logframe-output.jpg', cframe_rgb)
