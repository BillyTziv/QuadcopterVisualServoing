...
for frame in camera.capture_continuous(rawCapture, format="bgr", use_video_port=True):
	...
	cframe_rgb = cv2.imread('input.png', cv2.IMREAD_COLOR)
	image = cframe_rgb
	
	# Find the mix and max values of the pixels in each channel
	bMin = image[..., 0].min()
	bMax = image[..., 0].max()
	gMin = image[..., 1].min()
	gMax = image[..., 1].max()
	...
	print "Red min: ", rMin
	print "Red max: ", rMax
	print "Green min: ", gMin
	print "Green max: ", gMax
	print "Blue min: ", bMin
	print "Blue max: ", bMax
	...
	# Create new images with RGB layers
	img1 = image.copy()
	img1[..., 1] = 0
	img1[..., 2] = 0
	cv2.imwrite('blueImage.jpg', img1)

	img2 = image.copy()
	img2[..., 0] = 0
	img2[..., 2] = 0
	cv2.imwrite('greenImage.jpg', img2)

	
	img3 = image.copy()
	img3[..., 0] = 0
	img3[..., 1] = 0
	cv2.imwrite('redImage.jpg', img3)

	rawCapture.truncate(0)
	# Apply a blur filter (optional)
	# cframe_blur = cv2.medianBlur(cframe_gray, 5)
	
	height, width, channels = cframe_rgb.shape
	print "Image height: ", height
	print "Image width: ", width
	print "Total channels found: ", channels

	blueList=[]
	redList=[]
	blackList=[]
	greenList=[]
	print "First channel total elements: ", len(cframe_rgb[..., 0])
	print "Second channel total elements: ", len(cframe_rgb[..., 1])
	print "Third channel total elements: ", len(cframe_rgb[..., 2])
	
	print "Blue: ", cframe_rgb[..., 0]
	print "Green: ", cframe_rgb[..., 1]
	print "Red: ", cframe_rgb[..., 2]
	#print "Blue channel: ", cframe_rgb[0, 0, 0]

	#print "Green channel: ", cframe_rgb[..., 1]
	print "Red list: ", redList
	
	#print "Red list: ", redList
	break
	blueList = np.argwhere(cframe_rgb[..., 0][0] > 15)
	greenList = np.argwhere(cframe_rgb[..., 0][1] > 15)
	blackList = np.argwhere(sum(cframe_rgb) < 15)
	print "Red list: ", len(redList)
	print redList
	print redList[0]
	print redList[100]
	print "Green list: ", len(greenList)
	print "Blue list: ", len(blueList)
	print "Black list: ", len(blackList)
	break
	
	for i in range(0, 480):
		for j in range(0, 640):
			pixel = cframe_rgb[i, j]
			mValue = max(pixel)
			index = pixel.argmax(axis=0)

			if( index == 0 ):
				# Blue colour
				blueList.append(pixel)
			elif( index == 1 ):
				# Green colour
				greenList.append(pixel)
			elif( index == 2 ):
				# Red colour
				redList.append(pixel)
			elif(sum(pixel)/3 < 20):
				# Black colour
				blackList.append(pixel)

			
	print "Found # blue pixels: ", len(blueList)
	
	break


	indices = np.where(np.all(cframe_rgb == (255, 255, 100), axis=-1))
	print indices
	print zip(indices[0], indices[1])
	#print redList
	greenList = np.argwhere(cframe_rgb < [0, 20, 0])

	#print #greenList
	blueList = np.argwhere(cframe_rgb == [0, 0, 100])
	print blueList[0]
	print len(blueList)

	blackList = np.argwhere(cframe_rgb < [20, 0, 0])
	break
	counter = 1
	if len(blackList) == 0:
		continue	
	...
	s_prev = s
        ...
        if len(s) == 4:
		# P00repare the string for the arduino (must start with '<' and end with '>')
