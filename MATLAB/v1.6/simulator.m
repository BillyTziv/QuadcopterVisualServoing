...
endPoint = [5, 2, 10];          % Desired final position  x, y, z
...
F_des(1) = (m*g)+k*(endPoint(3)-altitude(3, 1))+kd*(0-v_out(3, 1));

% Torque on X axis
torque(1, 1) = ktx*(endPoint(1)-altitude(1, 1))+ktx*(0-v_out(1, 1));

% Torque on Y axis
torque(2, 1) = kty*(endPoint(2)-altitude(2, 1))+kty*(0-v_out(2, 1));

% Torque on Z axis
torque(3, 1) = ktz*(theta(3, 1)-theta(3, 1));

vector= [F_des(1), torque(1, 1), torque(2, 1), torque(3, 1)]';
gamma = [ct, ct, ct, ct; 0, d*ct, 0, -d*ct; -d*ct, 0, d*ct, 0; -cq, cq, -cq, cq];
eng_omega(:, 1) = sqrt(gamma\vector);

% Acceleration of the craft
R = rotation(theta);
thrust(:, 1) = [0; 0; ct * sum(eng_omega(:, 1).^2)];
weight = [0;0;-m*g;];
