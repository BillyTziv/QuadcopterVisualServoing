...
% phi, psi, theta.
function old_vis(xin, yin, zin, x_out, v_out, a_out, times)
    subplot(4,3,1)
    grid off;
    plot(times, x_out(3, :), 'r', 'LineWidth', 2);
    ylabel('Position');
    
    
    subplot(4,3,4)
    grid off;
    plot(times, v_out(3, :), 'y', 'LineWidth', 2);
    ylabel('Velocity');
    
    
    subplot(4,3,7)
    grid off;
    plot(times, a_out(3, :), 'b', 'LineWidth', 2);
    ylabel('Acceleration');
