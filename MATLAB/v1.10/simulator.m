...
ki = 1;                        % PID controller integral gain
...
%     end
    T = [0.001 0.0001 m*g]' + kp*e + ki*error_sum + kd*edot;

    % Torque controller
    vel_x_des = kvx*(endPoint(1) - x(1));
    theta_x_des = (m/T(3))*kthetax*(vel_x_des - v(1));
   
    vel_y_des = kvy*(endPoint(2) - x(2));
    theta_y_des = (m/T(3))*kthetay*(vel_y_des - v(2))
    
  
    torque(1) = 0;%kttx*(theta_x_des - theta(3)) - ktx*thetadot(3);
    ...
    vector = [T(3), torque(1), torque(2), torque(3)]';
