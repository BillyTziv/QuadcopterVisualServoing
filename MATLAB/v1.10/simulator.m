...
kfx = 0.1;                        % Desired force position constant
kfv = 100;                        % Desired force velocity constant
...
for index = 1:1:N
    ...
    F_des = (m*g)+kfx*(endPoint(3)-x(3))-kfv*v(3);

    % Torque controller
    vel_x_des = kvx*(endPoint(1) - x(1));
    theta_x_des = (m/F_des)*kthetax*(vel_x_des-v(1));
    
    vel_y_des = kvy*(endPoint(2) - x(2));
    theta_y_des = (m/F_des)*kthetay*(vel_y_des-v(2));
    
  
    torque(1) = kttx*(theta_x_des - theta(3)) - ktx*thetadot(3);
    torque(2) = 0;%ktty*(theta_y_des - theta(2)) - kty*thetadot(2);
    ...
    engine_RPM = sqrt(abs(gamma\vector)); % kw
    R = rotation(theta);
    thrust = [0; 0; ct * sum(engine_RPM.^2)];
...
data = struct('x', x_out, 'v', v_out, 'a', a_out, 'torque', torque, 'theta', theta_out,...
