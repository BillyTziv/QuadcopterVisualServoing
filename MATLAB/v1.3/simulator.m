...
endPoint = [0, 0, 50];          % Desired final position  x, y, z (0, 0, 100)
...
for index = 1:10:length(times)
    %visualize(times, x_out, v_out, a_out);
    old_vis(x_out(1, index), x_out(2, index), x_out(3, index), x_out, v_out, a_out, times, index, endTime, F_des, eng_omega);
    drawnow;
    
    % Delete all the previous graphics and ouput some new data on the same
    % figure window
